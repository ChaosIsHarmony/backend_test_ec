PRELUDE:
    ...
    respective services through a Docker network on the localhost. The only assumed software on the examiner's computer
    is the Docker Engine.

    In the interest of time, I have omitted several things that were not specifically requested in the PDF--most notably,
    tests, security (e.g., authentication, hardcoded passwords/API keys, etc.), and a good logger. I did, however,
    attempt to code for extensibility and believe these could be easily added (e.g., passwords in a .env file).
...
TO RUN:
    ...
    NOTE: if the above fails, the contents of the script are essentially as below.
...
TO TEST API INTEGRATION:
    ...
    FETCH A PET FOR CUSTOMER
        ...
        $ curl http://localhost:8080/customer/add -H "Content-Type: application/json" -d '{"firstName":"Bobo", "lastName":"McGee", "email":"bobo.m@b.com", "id":-1}'
        $ curl http://localhost:8080/customer/get_pet_for_customer/2